...
> 让每一个经营决策，都先在未来发生一次。
> LLM 驱动的多智能体图谱推演沙盘（代号 GodView）— 输入一个零售假设，长出一个会演化、能解释、可访谈的世界。

Dmall Future Market 把"零售决策推演"变成三步叙事：**WHAT IF（假设）→ SIMULATE（涌现）→ OBSERVE（洞察）**，外加常驻的 **INTERVIEW（访谈）**。图谱上的每个节点都是一个带人格的 agent，整个推演过程可见、可问、可追溯。应用打开先见品牌首页，点「开始推演」进入工作台，左上角品牌可随时返回首页。
...
| **Interview 节点** | 与任意图谱节点以 persona + 推演行为记忆对话（角色扮演） |
| **场景包制** | 领域知识外置为配置（零售/供应链已内置），不同场景 = 不同输入，推理引擎恒定 |
| **D3 知识图谱** | 力导向布局 + 点阵星空背景；动态稳定节点配色（新类型自动领色、同类型恒同色）；悬停只高亮不模糊（玫粉高亮边）；搜索/路径/详情/图例 |
...
0. **首页 · 品牌落地页**：打开应用先见 Dmall Future Market 首页（深色科技感），点「开始推演 / 进入工作台」进入工作台，点左上角品牌 logo 随时返回首页
